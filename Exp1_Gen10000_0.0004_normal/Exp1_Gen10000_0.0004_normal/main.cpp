...
void  measure()
{
	...
	for (int i = 0; i < partical_num; i++) {
		...
		for (int j = 0; j < s_stock_index; j++) {
			...
			//cout << fixed << setprecision(0) << partical[i][j]; //Partical
		}
		//cout << n[i] << endl;
	}
	//system("pause");
...
void standardization()
{
	...
	for (int i = 0; i < partical_num; i++)
	{
		...
		for (int j = 0; j < n[i]; j++) {
			...
			for (int k = 0; k < day; k++) {
				...
				else
				{
					...
					//cout << n_fund_standardization[i][j][k] << endl;
				}
			}
		}
		...
		for (int k = 0; k < day; k++)
		{
			...
			//final_fund[i] = yi[i][day - 1];
			//cout << fixed << setprecision(15) << yi[i][250] << endl;
		}

		//all_final_yi[i] = yi[i][day];
		//cout << fixed << setprecision(15) << all_final_yi[i] << endl;
		//cout << fixed << setprecision(15) << yi[i][250] << endl;
...
void test_standardization()//測試期資金水位計算
{
	...
	/*for (int j = 0; j < Gbest_n; j++)
	{
	last_test_all_buy_paper[j] = 0;
	for (int s = 0; s < test_stock_index; s++)
	{
	for (int v = 0; v < s_stock_index; v++)
	{
	...
	if (final_portfolio[v] == test_stock_no[s])
	{
	last_test_all_buy_paper[j] = test_all_buy_paper[s];
	...
	//cout << last_test_all_buy_paper[j] << endl;
	//return;
...
void experiment_compre() /*50次實驗Gbest比較,並找出Gbest的實驗數*/
{
	...
	cout << "最佳解實驗:" << best_experimrentime << endl;
}
...
void test_check()
{
	...
	cout << "風險:" << test_risk << endl;
	cout << "預期報酬:" << test_m << endl;
	cout << "測試期趨勢值:" << test_trend_ratio << endl;
	cout << "檔數:" << Gbest_n << endl;
	cout << "初始資金:" << test_initial_fund << endl;
	cout << "最後資金:" << fixed << setprecision(10) << test_all_final_fund << endl;
...
void out_file(int a)
{
	string ouput_file = "Larry_result_" + M2M_train[a].substr(0, M2M_train[a].length());//輸出檔案名稱
		...
		for (int v = 0; v < s_stock_index; v++)
		{
			if (final_portfolio[s] != 0)
			{
				if (final_portfolio[v] == test_stock_no[s])
				{
					...
					//cout << test_all_buy_paper[s] << endl;
				}
			}
		}
	}
...
/*void out_file()
{
/*output_file.open("Y2Y_2010_beta.csv", ios::out);
output_file << fixed << setprecision(5);*/


/*output_file << "Beta" << "," << "Gen" << "," << t + 1 << endl;
for (int i = 0; i < s_stock_index; i++)
{
output_file << beta[i] << ",";
}*/
...
{
output_file << "Generation :" << "," << k + 1 << endl;
	...
	for (int a = 0; a < file_num; a++)
	{
		...
		for (int j = 0; j < experiment_time; j++)
		{
			...
			for (int t = 0; t < generation; t++)
			{
				...
				/*if (a == 3)
				{
				output_file.open("train_2011_Q1-Q2_betacheck.csv", ios::app);
				output_file << "實驗# " << "," << experiment_time << "," << "世代# " << "," << t + 1 << endl;
				for (int i = 0; i < s_stock_index; i++)
				{
				output_file << beta[i] << ",";
				}
				...
				/*if (a == 3)
				{
				output_file.open("train_2011_Q1-Q2_particalcheck.csv", ios::app);
				output_file << "第" << t + 1 << "代" << endl;
				for (int j = 0; j < partical_num; j++)
				{
				output_file << "Partical" << "," << all_trend_ratio[j] << ",";
				for (int i = 0; i < s_stock_index; i++)
				{
				output_file << partical[j][i] << ",";
				}
		...
		{
			test_initial_fund = test_all_final_fund;
		}
